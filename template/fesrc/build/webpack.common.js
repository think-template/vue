...
const staticBase = path.resolve(__dirname, '../../server/www/static')
const viewBase = path.resolve(__dirname, '../../server/view')
...
const logger = new Logger({
  ...
  layout: { type: 'pattern', pattern: `%[[%r]%] ${colors.yellow('webpack')} - %[%m%]` }
});
...
fileList.forEach((filename) => {
  const name = pathPosix.relative(entryBase, filename).replace(/\.js$/, '')
  ...
  htmlWebpackPlugins.push(new HtmlWebpackPlugin({
    ...
    chunksSortMode: function (a, b) {
      let orders = ['runtime', 'common', 'global', name];
...
const copyIncHtmlWebpackPlugins = incFileList.map((filename) => {
  const name = pathPosix.relative(entryBase, filename)
...
module.exports = {
  ...
  resolve: {
    ...
    alias: {
      ...
      // 常用
      _vue: path.resolve(__dirname, '../../fesrc/node_modules/vue/dist/vue.common.js'),
    },
    enforceExtension: false,
    modules: [
      path.resolve(__dirname, '../../fesrc/js'),
      'node_modules',
    ],
  },
}
